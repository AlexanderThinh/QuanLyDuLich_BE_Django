...
class TourViewSet(viewsets.ViewSet, generics.ListAPIView, generics.RetrieveAPIView):
    ...
    def booking(self, request, pk):
        # tour = Tour.objects.get(pk=pk)
        # loai_ve = request.data.get('loai_ve')
        # don_gia = request.data.get('don_gia')
        # so_luong = request.data.get('so_luong')
        #
        # if don_gia and so_luong and loai_ve:
        #     lv = LoaiVe.objects.get(pk=loai_ve)
        #     ddt = DonDatTour.objects.create(loai_ve=lv, don_gia=don_gia,
        #                                     so_luong=so_luong, khach_hang=request.user,
        #                                     tour=self.get_object())
        #     tour.dondattour.add(ddt)
        #     tour.save()
        #
        #     return Response(data=DonDatTourSerializer(ddt).data,
...
class TinTucViewSet(viewsets.ViewSet, generics.ListAPIView,
    ...
    def get_permissions(self):
        if self.action in ['like', 'add_comments']:
            return [permissions.IsAuthenticated()]
        return [permissions.AllowAny()]
